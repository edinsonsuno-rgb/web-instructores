...
export default function ResetPasswordPage() {
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    setLoading(false)
    if (error) {
      toast.error('Error al actualizar. Solicita un nuevo link.')
    } else {
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="df-card p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-df-muted text-sm mt-1">Elige una contraseña segura</p>
          </div>

          {!ready ? (
            <div className="text-center py-6">
              <div className="w-8 h-8 border-4 border-df-border border-t-df-violet rounded-full animate-spin mx-auto mb-3"/>
              <p className="text-df-muted text-sm">Verificando link...</p>
            </div>
          ) : (
            <form onSubmit={handleReset} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-df-muted mb-2">Nueva contraseña</label>
